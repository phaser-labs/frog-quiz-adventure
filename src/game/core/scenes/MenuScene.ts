...
export class Menu extends Phaser.Scene {
  ...
  create() {
    ...
    const svgElement = this.add.dom(400, 120).createFromHTML(svgContent);
    ...
    // Crear el modal directamente en el body del documento, NO en Phaser
    const modalHTML = `
    <div class="gameFrog__containerSettingshtml" id="settingsModal" style="display: none;">
      <div class="gameFrog__containerSettings">
        <button class="gameFrog__btnClose" aria-label="Cerrar modal">
    ...
    const gameContainer = document.querySelector('.gameFrog__container');
    if (gameContainer) {
    ...
    const btnClose = document.querySelector('.gameFrog__btnClose') as HTMLElement;
    ...
    function handleClickInformation() {
      ...
      if (ocultar2) {
        if (openInformation) {
          ...
          ocultar2.style.transition = 'height 0.3s ease';
    ...
    function closeModal() {
      ...
      btnConfig.setAttribute('aria-pressed', 'false');
    }
    ...
    const botonConfig = this.add.dom(width - 30, height - 25, 'button', '', '') as Phaser.GameObjects.DOMElement;
    ...
    btnConfig.addEventListener('click', () => {
      if (settingsModal.style.display === 'none' || settingsModal.style.display === '') {
        ...
        btnConfig.setAttribute('aria-pressed', 'true');
      } else {
        closeModal();
      }
    });
    ...
    function loadSwitchState() {
      ...
      if (savedState !== null && toggleSwitch) {
    ...
    function loadSwitchState2() {
      ...
      if (savedState !== null && toggleSwitch2) {
