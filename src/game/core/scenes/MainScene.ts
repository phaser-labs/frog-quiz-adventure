...
let vidas = 4; // variable para las vidas del jugador
let puntos = 0; // variable para los puntos del jugador
...
export class Main extends Phaser.Scene {
      ...
      btn.addEventListener('click', (event) => {
        ...
        this.time.delayedCall(4000, () => {
          ...
          if (resetearPosiciones && this.currentIndex < globalState.data.length) {
            this.renderPregunta(this.currentIndex, hojasConHover, rana);
            this.oculatarHojas();
  ...
  create() {
    ...
    this.renderPregunta(this.currentIndex, hojasConHover, this.rana);
  }
